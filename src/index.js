...
// ============================================================
// /unbanme — débannissement depuis un autre serveur
// ============================================================

async function handleUnbanMeCommand(interaction) {
  if (!interaction.inGuild()) {
    await interaction.reply({
  ...
  const allowedUserId = process.env.UNBAN_OWNER_ID?.trim();
  const targetGuildId = process.env.UNBAN_TARGET_GUILD_ID?.trim();

  if (!allowedUserId || !targetGuildId) {
    await interaction.reply({
      content:
        '❌ UNBAN_OWNER_ID ou UNBAN_TARGET_GUILD_ID n’est pas configuré dans le fichier .env.',
      flags: MessageFlags.Ephemeral,
    });
  ...
  try {
    ...
    if (!targetGuild) {
      await interaction.editReply(
        '❌ Le bot n’est pas présent sur le serveur cible.',
      );

      return;
    }

    const botMember = await targetGuild.members
      .fetchMe()
      .catch(() => null);
    ...
    if (
      ...
      await interaction.editReply(
        '❌ Le bot n’a pas la permission **Bannir des membres** sur le serveur cible.',
      );

      return;
    }

    const ban = await targetGuild.bans
      .fetch(interaction.user.id)
      .catch(() => null);

    if (!ban) {
      await interaction.editReply(
        `ℹ️ Tu n’es actuellement pas banni de **${targetGuild.name}**.`,
      );
    ...
    await targetGuild.bans.remove(
      interaction.user.id,
      `Débannissement demandé via /unbanme par ${interaction.user.tag}`,
    );

    await interaction.editReply(
      `✅ Tu as été débanni de **${targetGuild.name}**.`,
    );
  } catch (error) {
    console.error('Erreur /unbanme :', error);

    await interaction.editReply(
      '❌ Impossible de te débannir. Vérifie les permissions du bot et les logs.',
    );
  }
}
...
// ============================================================
// Ajout de /unbanme au gestionnaire de commandes existant
// ============================================================
